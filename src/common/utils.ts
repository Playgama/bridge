...
interface ProgressLogoPathItem {
    d: string
    fillRule?: string
    maskFill?: string
}

interface ProgressLogoPreset {
    viewBox: string
    paths: Array<string | ProgressLogoPathItem>
    fillColor: string
    strokeColor: string
    gradientStops: Array<{ offset: string; color: string }>
    gradientWidthMultiplier?: number
}

export function createProgressLogo(showFullLoadingLogo: boolean, showLoadingText = false): void {
    const style = document.createElement('style')
    style.textContent = `
        .fullscreen {
            background: #242424;
    ...
    const defaultPreset: ProgressLogoPreset = {
        viewBox: '0 0 633 819',
        paths: [
    ...
    const fullBridgePreset: ProgressLogoPreset = {
        viewBox: '0 0 633 986',
        paths: [
    ...
    gradRect.setAttribute('width', String(vbWidth * (resolved.gradientWidthMultiplier ?? 1)))
